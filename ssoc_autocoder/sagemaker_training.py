...
def prepare_model(encoding, parameters):
    ...
    model = HierarchicalSSOCClassifier_V2pt2(parameters,encoding)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train', type=str, default="s3://ag-mom-alphatest/train/data/train.csv")
    parser.add_argument('--test', type=str, default="s3://ag-mom-alphatest/train/data/test.csv")
    ...
    encode_train = encode_dataset(train_data, encoding, colnames={'SSOC':'SSOC', 'job_title':'job_title', 'job_description':'job_description'})
    test_data = pd.read_csv(args.test)
    encode_test = encode_dataset(test_data, encoding, colnames={'SSOC':'SSOC', 'job_title':'job_title', 'job_description':'job_description'})
    ...
    training_loader_distilbert, validation_loader_distilbert = prepare_data(encode_train, encode_test, tokenizer_distilbert, colnames={'SSOC':'SSOC', 'job_title':'job_title', 'job_description':'job_description'}, parameters = training_param_distilbert_hierarchical)
    ...
    with load_run(sagemaker_session=session, experiment_name="ys-ssoc-training-job-experiment", run_name="experiment-run-train-5epochs") as run:
        run.log_parameters(
    ...
    copy_tree("/opt/ml/code/", "/opt/ml/model/")
    torch.save(model.state_dict(), os.path.join('/opt/ml/code/model', 'model.pth'))
