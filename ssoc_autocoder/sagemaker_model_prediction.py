...
from ssoc_autocoder import sagemaker_model_training
import pickle
import numpy as np
...
#     text_mask = torch.tensor([tokenized_text['attention_mask']], dtype = torch.long).to(device)
    ...
    try:
        ...
        model.eval()
        with torch.no_grad():
            to_str = json.loads(tensor)
            numpy_array = np.array(to_str)
            tensor_data = torch.tensor(numpy_array, dtype=torch.float).to(device)
        ...
        for ssoc_level, ssoc_level_params in sorted(ssoc_prediction_parameters.items()):
            ...
            # return preds[ssoc_level].detach().cpu().numpy().argsort()[0]

            if ssoc_level != "SSOC_5D":
                # Extract the indices of the top n predicted SSOCs for the given SSOC level
                predicted_idx = preds[ssoc_level].detach().cpu().numpy().argsort()[::-1][:ssoc_level_params["top_n"]]
                ...
                predicted_idx = preds[ssoc_level].detach().cpu().numpy().argsort()[::-1][:top_5D_n]
            ...
            predicted_proba_all = m(preds[ssoc_level]).detach().cpu().numpy()
        ...
        return f'{predictions_with_proba}here',e
